...
def reinforce(game, policy_model, target_model, policy_criterion, target_criterion, policy_optimizer, target_optimizer,
    ...
    for episode in range(num_episodes):
        ...
        for t, transition in enumerate(episode_info[:-1]):
            ...
            picked_action = pred_return.gather(0, torch.Tensor([transition.action]).type(torch.int64).to(device))
            ...
            policy_loss = -torch.log(picked_action) * total_return * gamma**t
...
def q1(game):
    ...
    policy_optimizer = torch.optim.SGD(policy_model.parameters(), lr=0.00001)
    target_optimizer = torch.optim.SGD(target_model.parameters(), lr=0.00001)
