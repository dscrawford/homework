...
double getDiscount(int quantity) { //Determines the discount the user will receive
    double discount;
    if(quantity >= 16 && quantity <= 29) {
        discount = 0.10;
    }
    else if(quantity >= 30 && quantity <= 45) {
        discount = 0.15;
    }
    else if(quantity >= 46 && quantity <= 89) {
        discount = 0.20;
    }
    else if(quantity >= 90){
        discount = 0.25;
...
bool isPreferred(int quantity) {
    const int PREFERRED_CUSTOMER_LEVEL = 200; //If they meet this number, then the customer is a preferred customer
...
double calculateSale(int quantity, double discount) { //Calculates the total revenue the company will receive.
    const double RETAIL_PRICE = 96.0;//Retail price is the price of the unit
