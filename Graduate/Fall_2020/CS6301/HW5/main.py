...
class MobV2Bottleneck(nn.Module):

    # initialization
    def __init__(self, C_in, C_res, C_out, S):

        # parent initialization
        super(MobV2Bottleneck, self).__init__()
...
class Model(nn.Module):
    ...
    def __init__(self, data_num_channels, data_num_classes, model_level_0_blocks, model_level_1_blocks, model_level_2_blocks, model_tail_end_channels, model_level_0_identity_channels, model_level_0_residual_channels, model_level_1_identity_channels, model_level_1_residual_channels, model_level_2_identity_channels, model_level_2_residual_channels):
        ...
        self.enc_tail.append(nn.Conv2d(3, 32, (3, 3), stride=(1, 1), padding=(1, 1), dilation=(1, 1), groups=1, bias=False, padding_mode='zeros'))

        # encoder level 0
        self.enc_0 = nn.ModuleList()
        self.enc_0.append(MobV2Bottleneck(32, 32 * 1, 64, 2))
        for n in range(4 - 1):
            self.enc_0.append(MobV2Bottleneck(64, 32 * 1, 64, 1))

        # encoder level 1
        self.enc_1 = nn.ModuleList()
        self.enc_1.append(MobV2Bottleneck(64, 64 * 6, 96, 1))
        for n in range(3 - 1):
            self.enc_1.append(MobV2Bottleneck(96, 64 * 6, 96, 1))

        # encoder level 2
        self.enc_2 = nn.ModuleList()
        self.enc_2.append(MobV2Bottleneck(96, 96 * 6, 160, 2))
        for n in range(3 - 1):
            self.enc_2.append(MobV2Bottleneck(160, 96 * 6, 160, 1))

        # encoder level 3
        self.enc_3 = nn.ModuleList()
        self.enc_3.append(MobV2Bottleneck(160, 160 * 6, 320, 1))
        # for n in range(3 - 1):
        #     self.enc_3.append(MobV2Bottleneck(320, 160 * 6, 320, 1))

        # # encoder level 2 complete the (conv) - bn - relu pattern
        # self.enc_2.append(nn.BatchNorm2d(96, eps=1e-05, momentum=0.1, affine=True, track_running_stats=True))
        # self.enc_2.append(nn.ReLU())

        # decoder
        self.dec = nn.ModuleList()
        self.dec.append(nn.Conv2d(320, 1280, (1, 1), stride=(1, 1), dilation=(1, 1), groups=1, bias=False))
        self.dec.append(nn.AdaptiveAvgPool2d((1, 1)))
        self.dec.append(nn.Conv2d(1280, data_num_classes, (1, 1), stride=(1, 1), dilation=(1, 1), groups=1, bias=False))
